...
def convert_mcp_gateway_no_auth_to_porcelain(plumbing):
    if plumbing is None:
        return None
    porcelain = models.MCPGatewayNoAuth()
    porcelain.bind_interface = (plumbing.bind_interface)
    porcelain.egress_filter = (plumbing.egress_filter)
    porcelain.healthy = (plumbing.healthy)
    porcelain.hostname = (plumbing.hostname)
...
def convert_mcp_gateway_no_auth_to_plumbing(porcelain):
    plumbing = MCPGatewayNoAuth()
    if porcelain is None:
        return plumbing
    plumbing.bind_interface = (porcelain.bind_interface)
    plumbing.egress_filter = (porcelain.egress_filter)
...
def convert_repeated_mcp_gateway_no_auth_to_plumbing(porcelains):
    return [
...
def convert_mcp_gateway_pat_to_porcelain(plumbing):
    if plumbing is None:
        return None
    porcelain = models.MCPGatewayPAT()
    porcelain.bind_interface = (plumbing.bind_interface)
    porcelain.egress_filter = (plumbing.egress_filter)
    porcelain.healthy = (plumbing.healthy)
    porcelain.hostname = (plumbing.hostname)
...
def convert_mcp_gateway_pat_to_plumbing(porcelain):
    plumbing = MCPGatewayPAT()
    if porcelain is None:
        return plumbing
    plumbing.bind_interface = (porcelain.bind_interface)
    plumbing.egress_filter = (porcelain.egress_filter)
...
def convert_repeated_mcp_gateway_pat_to_plumbing(porcelains):
    return [
...
def convert_repeated_mcp_gateway_pat_to_porcelain(plumbings):
    return [
...
def convert_node_tcp_probe_response_to_porcelain(plumbing):
    if plumbing is None:
        return None
    porcelain = models.NodeTCPProbeResponse()
...
def convert_node_tcp_probe_response_to_plumbing(porcelain):
    plumbing = NodeTCPProbeResponse()
    if porcelain is None:
        return plumbing
...
def convert_repeated_node_tcp_probe_response_to_plumbing(porcelains):
    return [
        convert_node_tcp_probe_response_to_plumbing(porcelain)
        for porcelain in porcelains
    ]


def convert_repeated_node_tcp_probe_response_to_porcelain(plumbings):
    return [
        convert_node_tcp_probe_response_to_porcelain(plumbing)
        for plumbing in plumbings
    ]
